...
export default function VehiclesPage() {
  ...
  const [linkedUser, setLinkedUser] = useState<{ id: number; name: string; email: string } | null>(null);
  ...
  function handleRegChange(value: string) {
    const upper = value.toUpperCase();
    setForm(f => ({ ...f, registration: upper }));
    if (modal !== 'add') return;

    if (lookupTimer.current) clearTimeout(lookupTimer.current);
    const trimmed = upper.replace(/\s/g, '');
    if (trimmed.length < 2) { setLookupStatus('idle'); return; }
  ...
  async function save() {
    ...
    try {
      const payload = {
        ...
        ...(modal === 'add' && linkedUser ? { user_id: linkedUser.id } : {}),
      };
      if (modal === 'add') {
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full border border-slate-200 rounded-xl px-3 py-2 pr-10 text-sm font-mono uppercase outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50"
                  />
                  {modal === 'add' && (
                    <span className="absolute right-3 top-1/2 -translate-y-1/2">
                      {lookupStatus === 'loading' && <span className="w-4 h-4 border-2 border-blue-400 border-t-transparent rounded-full animate-spin inline-block" />}
                      {lookupStatus === 'found' && <span className="text-green-500 text-sm font-bold">✓</span>}
                    </span>
                  )}
                </div>
                {modal === 'add' && lookupStatus === 'found' && (
                  <p className="text-xs text-green-600 mt-1 font-medium">✓ Details auto-filled from order history</p>
                )}
                {modal === 'add' && lookupStatus === 'not_found' && form.registration.replace(/\s/g,'').length >= 3 && (
                  <p className="text-xs text-slate-400 mt-1">No previous orders found for this reg — fill in details manually</p>
                )}
                {modal === 'add' && linkedUser && (
                  <div className="mt-2 flex items-center gap-2 bg-blue-50 border border-blue-100 rounded-xl px-3 py-2">
                    <svg className="w-4 h-4 text-blue-400 shrink-0" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-blue-800 truncate">{linkedUser.name}</p>
                      <p className="text-xs text-blue-500 truncate">{linkedUser.email}</p>
                    </div>
                    <button onClick={() => setLinkedUser(null)} className="ml-auto text-blue-400 hover:text-blue-600 shrink-0">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                    </button>
                  </div>
                )}
